...
// dictates the speed of the bicycle. So if person A pedals at a speed of 5, and person B
...
// Write a function that returns the maximum possible total speed or the minimum possible
// total speed of all of the tandem bicycles ridden based on an input parameter, fastest.
...
// For example, if there are 4 riders (2 red-shirt riders and 2 blue-shirt riders) who
// have speeds of 1, 3, 4, 5, and if they're paired on tandem bicycles as follows:
...
const tandemBicycle = (redShirtSpeeds, blueShirtSpeeds, fastest) => {
  ...
  for (let i = 0; i < redShirtSpeeds.length; i++) {
    let redRider = redShirtSpeeds[i];
    let blueRider = blueShirtSpeeds[i];

    speeds += Math.max(redRider, blueRider);
  }
  return speeds;
};
