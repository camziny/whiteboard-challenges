...
// then the total waiting time if the queries were executed in the order of [5, 4, 1]
// would be (0) + (5) + (5 + 1) = 11.

// The first query of duration 5 would be executed immediately, so its waiting time would be 0,
// the second query of duration 1 would have to wait 5 seconds (the duration of the first query)
// to be executed, and the last query would have to wait the duration of the first two
// queries before being executed. Note: you're allowed to mutate the input array.
